...
function submitLoanFitForm() {
    ...
    grecaptcha.execute('6LdmCn0nAAAAANC8dQDeC3bko97zlloPkuFcyP7_', { action: 'submit' }).then(function(token) {
        ...
        $.ajax({
            url: 'https://linker.leadsync.com.my/api/loanfit/loanfit-form',
            type: 'POST',
            ...
            success: function (response) {
                ...
                $(".form-process").hide();
                $(".form-error").hide();
                $(".form-success").css({ display: "flex" }).show();
                
                window.location.href = 'thank-you-loanfit';
...
$(document).ready(function () {
    // Attach input listener to remove non-numeric characters in phone input
